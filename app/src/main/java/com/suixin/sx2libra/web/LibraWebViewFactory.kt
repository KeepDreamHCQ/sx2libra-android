...
class LibraWebViewFactory(
    ...
    fun create(
        ...
        configure(webView)
        installBridge(webView, messageListener)
        return webView
    }
     ...
     */
    fun installBridge(webView: WebView, messageListener: LibraWebMessageListener?) {
    ...
    private fun loadBridgeScript(context: Context): String? = try {
        context.assets.open(BRIDGE_ASSET_PATH).bufferedReader(Charsets.UTF_8).use { it.readText() }
    } catch (_: IOException) {
        null
    }
